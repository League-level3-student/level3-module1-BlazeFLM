...
public class RetroSun extends PApplet {
	...
	int sunRadius = 200; 
	float yFloat = WIDTH / 2;
	float xFloat = (WIDTH / 2) - sunRadius; 
	float h = 40;
	...
	public void draw() {
		...
		fill(sunColors[0]);
		noStroke();
		ellipse(400, 400, 2 * sunRadius, 2 * sunRadius);
		...
		if(yFloat < (HEIGHT / 2) - (sunRadius / 4)) {
			yFloat = (HEIGHT / 2) + sunRadius;
			h = 40;
		}
